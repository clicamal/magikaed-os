...
inline bool ata_ready(void) { return (inb(ATA_PRIM_IO + 7) & 0x40); }

inline bool ata_busy(void) { return (inb(ATA_PRIM_IO + 7) & 0x80); }

inline void ata_prepare(uint32_t lba, uint32_t nsectors) {
  while (ata_busy());
